...
> [!TIP]
> **Read this as a rendered page** → [rahulni.github.io/Indic_LLM/5_data_mixture_curriculum/site.html](https://rahulni.github.io/Indic_LLM/5_data_mixture_curriculum/site.html) — sticky contents, full-text search, cross-links and figures drawn from the same audit data. GitHub shows `.html` files as source, never rendered, so the file itself ([`site.html`](site.html), built by [`plan/build_site.py`](plan/build_site.py)) needs a host: GitHub Pages above, or open it locally, or [raw.githack.com](https://raw.githack.com/rahulni/Indic_LLM/main/5_data_mixture_curriculum/site.html) without any setup.
