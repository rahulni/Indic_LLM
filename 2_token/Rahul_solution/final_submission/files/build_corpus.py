#!/usr/bin/env python3
"""
Fetch India's Wikipedia page in English, Hindi, Telugu, and Marathi and convert
each to a *faithful* Markdown corpus.

"Faithful" = we keep every visible article character (links, URLs, tables,
references, image links, navboxes, categories). We strip only technical machinery
(script/style/meta tags). This matches the grading standard used by the trainer's
reference solution, so our fertility numbers are directly comparable.
...
USER_AGENT = "ERA V5 tokenizer assignment (rahul) /1.0"
FAITHFUL_UNIT_RE = regex.compile(r"[\p{L}\p{M}\p{N}]+|[^\s\p{L}\p{M}\p{N}]")
WORD_RE = regex.compile(r"[\p{L}\p{M}\p{N}]+")

# India's page in each language (title in that Wikipedia).
PAGES = {
    "en": ("English", "India"),
    "hi": ("Hindi", "भारत"),
    "te": ("Telugu", "భారతదేశం"),
...
def build_one(lang: str, title: str) -> dict:
    ...
    meta = {
        ...
        "unit_policy": "One unit per contiguous Unicode letter/mark/number run and per visible non-space punctuation/symbol character.",
    }
...
def main() -> int:
    for code, (name, title) in PAGES.items():
        ...
        print(f"{code} {name}: {meta['words']} words, {meta['faithful_units']} faithful units, {meta['chars']} chars")
    return 0
