#!/usr/bin/env python3
"""
Evaluate the saved faithful-BPE tokenizer:
  ...
  * PROVE faithfulness: decode(encode(text)) == text on every corpus AND on
    ...
    case the reference [UNK] tokenizer fails.
...
def main() -> int:
    ...
    print(f"\n  faithful spread = {spread:.4f}   SCORE = {1000/spread:.1f}   (reference = 6502.56)")
    ...
    print("\n  round-trip on out-of-corpus probes (reference [UNK] tokenizer drops these):")
    for s in PROBES:
